...
int _form_textarea_draw(component_t *self) {
    ...
    if (self->data != NULL) {
        ...
        field_opts_on(data->field, O_WRAP);
...
component_t *form_textarea_create(field_data_t *data) {
    ...
    if ((self = component_create(data->height, data->width, NULL, 0))) {
        ...
        self->type = COMPONENT_T_FORM_TEXTAREA;
        ...
        SET_ITEM(items[1], COMPONENT_M_DRAW, &_form_textarea_draw, 0, NULL);
