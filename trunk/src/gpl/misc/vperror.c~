...
void vperror_init(void (*stuff)(error_trace_t *error)) {
...
{    /* Local variables. */
    ...
    /* On the first call to this function with the global VPERROR_PRINT 
       flag set to false, check to see if environment variable, 
       VPERROR_PRINT, is defined. Waiting until the flag is false before 
       checking the environment variable allows this mechanism to work 
       for programs that enable error output during initialization and 
       disable it in subsequent processing. */
