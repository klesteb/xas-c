...
on block and relative files. Record locking is provided by the selective UNIX
file locking routines. There is also a btree implementation to handle 
indexing of files. This class is not compatible with actual DEC RMS files.
